...
<!--          </Button>-->
      ...
      <!-- Results Summary -->
      <div v-if="hasMemories" class="text-sm text-slate-600 dark:text-slate-400">
        Showing {{ memories.data.length }} of {{ memories.total }} memories
        <span v-if="hasFilters">(filtered)</span>
      </div>

      <!-- Memories List -->
      <div v-if="hasMemories" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card
...
<!--              </DropdownMenu>-->
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs text-slate-500">
              ...
              <div v-if="memory.document_type" class="flex items-center gap-1">
                <Folder class="h-3 w-3" />
                {{ memory.document_type }}
              </div>

              <div v-if="memory.project_name" class="flex items-center gap-1">
                <Folder class="h-3 w-3" />
                {{ memory.project_name }}
              </div>
            </div>
          ...
          <CardContent class="pt-0">
            ...
            <!-- Tags -->
            <div v-if="memory.tags && memory.tags.length > 0" class="flex flex-wrap gap-1 mb-3">
              <Badge
...
<!--            </div>-->
      ...
      <div v-else class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <div class="mt-6 flex gap-2">
          <Button v-if="hasFilters" @click="clearSearch" variant="outline">
            Clear Filters
          </Button>

          <Button as-child>
            <Link :href="memoriesIndex({ query: { create: '1' } }).url" class="gap-2">
              <Plus class="h-4 w-4" />
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <Button
            ...
            size="sm"
          >
            <Link :href="memories.prev_page_url">
              Previous
            </Link>
          </Button>
          ...
          <Button
            ...
            size="sm"
          >
            <Link :href="memories.next_page_url">
              Next
            </Link>
          </Button>
        </div>
